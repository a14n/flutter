...
class MissingInputException implements Exception {
  ...
  String toString() {
    ...
    return '$files were declared as an inputs, but did not exist. Check the definition of target:$target for errors';
  }
}
...
class CycleException implements Exception {
  ...
  @override
  String toString() =>
    'Dependency cycle detected in build: ${targets.map((Target target) => target.name).join(' -> ')}';
}
...
class MisplacedOutputException implements Exception {
  ...
  @override
  String toString() {
    return 'Target $target produced an output at $path which is outside of the current build or project directory';
  }
}
...
class MissingDefineException implements Exception {
  ...
  @override
  String toString() {
    return 'Target $target required define $define but it was not provided';
  }
}
